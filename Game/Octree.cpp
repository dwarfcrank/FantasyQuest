...
void Octree::MeshFromOctree(std::vector<u16>& indexBuffer, std::vector<Vertex>& vertexBuffer)
{
	auto t1 = std::chrono::high_resolution_clock::now();
...
void Octree::GenerateVertexIndices(Octree* node, std::vector<Vertex>& vertexBuffer)
{
	if (node && node->m_leaf)
	{
		glm::vec3 color;
		...
		Vertex v;
		v.Position.x = node->m_drawPos.x;
		v.Position.y = node->m_drawPos.y;
		v.Position.z = node->m_drawPos.z;
		v.Normal.x = node->m_averageNormal.x;
		v.Normal.y = node->m_averageNormal.y;
		v.Normal.z = node->m_averageNormal.z;
		v.Color.x = color.x;
		v.Color.y = color.y;
		v.Color.z = color.z;
		v.Color.w = 1.0f;
...
void Octree::CellChildProc(const std::array<Octree*, 8>& children, std::vector<u16>& indexBuffer)
{
...
void Octree::CellProc(std::vector<u16>& indexBuffer)
{
	if (!m_leaf)
	{
...
void Octree::EdgeProcXY(Octree* n0, Octree* n1, Octree* n2, Octree* n3, std::vector<u16>& indexBuffer)
{
	if (!n0 || !n1 || !n2 || !n3)
		return;
...
void Octree::EdgeProcXZ(Octree* n0, Octree* n1, Octree* n2, Octree* n3, std::vector<u16>& indexBuffer)
{
	if (!n0 || !n1 || !n2 || !n3)
		return;
...
void Octree::EdgeProcYZ(Octree* n0, Octree* n1, Octree* n2, Octree* n3, std::vector<u16>& indexBuffer)
{
	if (!n0 || !n1 || !n2 || !n3)
		return;
...
void Octree::FaceProcX(Octree* n0, Octree* n1, std::vector<u16>& indexBuffer)
{
	if (!n0 || !n1)
		return;
...
void Octree::FaceProcY(Octree* n0, Octree* n1, std::vector<u16>& indexBuffer)
{
	if (!n0 || !n1)
		return;
...
void Octree::FaceProcZ(Octree* n0, Octree* n1, std::vector<u16>& indexBuffer)
{
	if (!n0 || !n1)
		return;
...
void Octree::ProcessEdge(const Octree* node[4], int dir, std::vector<u16>& indexBuffer)
 {
	/*
